...
## Coding Agent Capability Benchmarks (Anthropic Internal Data, May 2026)

In May 2026, Anthropic published internal metrics on how Claude-assisted development had progressed over roughly one year ("When AI Builds Itself"). The figures are self-reported by a party with strong incentives to portray rapid progress (near an IPO filing), and the HN discussion raised substantive methodological concerns — particularly that code volume metrics conflate quantity with quality, and that a 40,000-line AI-assisted pull request that nobody reviews is not evidence of productivity. Practitioners reported receiving PRs they could not reasonably review, with code merged to main without understanding. These caveats apply to any interpretation of the numbers below.

With those caveats noted, several data points are the most concrete public figures available on large-scale coding agent deployment:

- **Task duration capability**: Claude progressed from completing tasks that take humans ~4 minutes to tasks taking ~12 hours, year-over-year. This is measured via METR's task duration benchmark.
- **Open-ended task success rate**: reached 76% in May 2026, up roughly 50 percentage points in six months (January–May 2026).
- **SWE-bench saturation**: models went from single-digit scores to benchmark saturation in approximately two years.
- **Code authorship**: over 80% of production code merged at Anthropic was attributed to Claude as of May 2026 (using an AI-attribution heuristic; accuracy caveated by Anthropic).
- **Research taste**: in an internal experiment, Claude's suggested experimental direction beat human researcher judgment 64% of the time in April 2026, up from 51% in November 2025. A 64% win rate is a modest edge; the direction of change is the more meaningful signal.

The article also articulates a distinction relevant to agent builders: current models are strong at **execution** — running well-specified tasks, writing code for defined problems — but weak at **judgment**: choosing which problems matter, setting research direction, deciding what to work on. This is sometimes called the "research taste" gap. In the context of coding agents, the practical implication is that agents are most reliable when the task is fully specified and the success criterion is verifiable, and least reliable when they must decide what to do at a high level. This mirrors the RLVR finding (agents improve most on tasks with binary verifiable outcomes) and the planning page's "Input Quality Is a Prerequisite" principle.
