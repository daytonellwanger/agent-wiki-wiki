...
A more hardened variant, used in Anthropic's open-source vulnerability discovery harness, layers gVisor (a user-space Linux kernel) over the container and enforces an egress allowlist — allowing outbound connections only to the Claude API and nothing else. This stops a compromised or misbehaving agent from exfiltrating data, downloading arbitrary payloads, or communicating with attacker infrastructure even if it escapes its immediate task scope. The harness enforces that autonomous code-executing agents refuse to start outside the sandbox entirely, making accidental unsandboxed execution a hard failure rather than a misconfiguration risk. For pipelines where agents run generated code or manipulate untrusted inputs (vulnerability research, fuzzing, automated patching), gVisor + egress allowlist is the published reference pattern for operator-enforced isolation.
...
A complementary lever is **KV-cache quantization**: reducing the numerical precision of the key-value cache entries that accumulate as context grows. Because the KV cache is stored in GPU memory, lower-precision representations translate directly into more context capacity per GPU and higher throughput at long context lengths. The challenge historically has been accuracy degradation and the need for per-model calibration passes. KVarN (Huawei CSL, June 2026) demonstrated that a pipeline of Hadamard rotation (spreading per-channel outliers), iterative variance normalization via a Sinkhorn-like algorithm, and asymmetric quantization can achieve 3–5x more KV-cache capacity with FP16-level accuracy (59.3% vs. 59.4% on AIME25 on Qwen3-32B at 16K context), no calibration required, and approximately 1.3x throughput improvement over FP16 baselines. It runs as a native vLLM fork activated by a single flag (`--kv-cache-dtype kvarn_k4v2_g128`). The technique is relevant to agents that operate on long contexts: more cache capacity means longer effective context windows on fixed hardware, which directly addresses the context pressure described in [Context Management](context-management.md).
