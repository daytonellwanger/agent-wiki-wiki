...
A significant design option in Skill frontmatter is `disable-model-invocation: true`. When set, the skill only runs when explicitly typed — the model cannot decide on its own that the skill is relevant and invoke it automatically. This is the right choice for skills with side effects (deploying, shipping, pushing) where you want explicit human intent, not model judgment. It trades the convenience of automatic activation for the safety of deliberate invocation.
