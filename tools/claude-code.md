...
A second enterprise response pattern emerged in June 2026 when Uber was reported to have capped all employees at $1,500/month per AI coding tool (applied per tool, so an engineer using two tools has separate $1,500 budgets for each). The context: Uber exhausted its entire 2026 AI budget within four months of the year. Simon Willison analyzed the cap as a sensible governance mechanism — preferable to either cutting access or encouraging unlimited usage — and noted it implies roughly $36,000/year per engineer assuming two tools, about 11% of median Uber SWE compensation. His own usage ran ~$1,000/month per provider, suggesting the cap is permissive for typical heavy individual users but constraining for agentic workflows that can run autonomously at scale. The HN discussion surfaced a related cost strategy: routing the majority of coding tasks to smaller "flash" models (which are ~10x cheaper and faster) and reserving frontier models for code review and complex reasoning, rather than defaulting all tasks to the most capable model.
